...
class Ptt(object):
    ...
    def is_success(self):
        ...
        if u"請按任意鍵繼續" in self._content:
            logging.info("in front page now...")
    ...
    def input_user_password(self):
        if u"請輸入代號" in self._content:
            logging.info('input acct'.format(self._user))
            self._telnet.write(self._user + b"\r\n")
            logging.info('input pw')
    ...
    def is_connect(self):
        ...
        if u"系統過載" in self._content:
            logging.info('server overload , try later')
            sys.exit(0)
        return True
    ...
    def login(self):
        if self.input_user_password:
            ...
            return True
        logging.info("ptt down, login fail")
        return False
...
def main():
    ...
    for i, user in enumerate(users):
        ...
        time.sleep(1)
        if ptt.is_connect():
            logging.info("using acct {}".format(user))
            if ptt.login():
                pass
        ptt.logout()
    logging.info("done")
